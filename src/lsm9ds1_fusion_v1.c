...
lsm9ds1_measurement_t lsm9ds1_read_gyro_integration() {
  ...
  if (measure.x_axis > 0.5 || measure.x_axis < -0.5) {
    integrated_angle.x_axis = GYRO_W * (integrated_angle.x_axis + measure.x_axis*time_diff) - ACCEL_W * angle.theta;
  }
  if (measure.y_axis > 0.5 || measure.y_axis < -0.5) {
    integrated_angle.y_axis = GYRO_W * (integrated_angle.y_axis + measure.y_axis*time_diff) + ACCEL_W * angle.psi;
  }
