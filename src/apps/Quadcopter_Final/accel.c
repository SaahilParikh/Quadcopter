...
void calibrate_accel() {
	...
	for(int j = 0; j < num; j++) {
		for(int i = 0; i < NUM_ELEMENTS; i++) {
			ret_code_t error_code = nrfx_saadc_sample_convert(Z_CHANNEL, &val);
			APP_ERROR_CHECK(error_code);
			val = MEDIANFILTER_Insert(&medianFilter, (float) val);
		}
		count += val;
	}
	printf("%lu, %f\n", (count)/num, (count*(ADC_VOLTAGE_RANGE/ADC_BITS)) / num);
	return;
...
void calibrate_accel2() {
	...
	for(int j = 0; j < num; j++) {
		  gx += (((float)sample_accel_value(X_CHANNEL)*(ADC_VOLTAGE_RANGE/ADC_BITS) - VX_0)/VX_D) - GX_0;
		 gy += (((float)sample_accel_value(Y_CHANNEL)*(ADC_VOLTAGE_RANGE/ADC_BITS) - VY_0)/VY_D) - GY_0;
		   gz += (((float)sample_accel_value(Z_CHANNEL)*(ADC_VOLTAGE_RANGE/ADC_BITS) - VZ_0)/VZ_D) - GZ_0;
	}
	return;
