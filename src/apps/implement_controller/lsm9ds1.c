...
lsm9ds1_measurement_t lsm9ds1_read_gyro_integration() {
  ...
  if (measure.z_axis > 0.5 || measure.z_axis < -0.5) {
    integrated_angle.z_axis = 1 * (integrated_angle.z_axis + measure.z_axis*time_diff); //+ 0.02 * magang.yaw;
  }
  if (measure.x_axis > 0.5 || measure.x_axis < -0.5) {
    integrated_angle.x_axis = GYRO_W * (integrated_angle.x_axis + measure.x_axis*time_diff) + ACCEL_W * angle.theta + MAG_W * magang.roll;
  }
  if (measure.y_axis > 0.5 || measure.y_axis < -0.5) {
    integrated_angle.y_axis = GYRO_W * (integrated_angle.y_axis + measure.y_axis*time_diff) + ACCEL_W * angle.psi + MAG_W * magang.pitch;
  }
